...
def verify_p24_tables():
    ...
    p24_paths = list(Path("data/images").glob("p24_*.md"))
    p24_path = p24_paths[0]

    with open(p24_path, "r", encoding="utf-8") as f:
        ocr_text = f.read()
    ...
    for i, row in enumerate(all_rows):
        ...
        if (
            "process start shift" in row_str or "17th" in row_str
        ) and "PROCESS_SHIFT" not in header_data:
            val = find_val(row, "process start shift") or find_val(row, "17th")
            header_data["PROCESS_SHIFT"] = val
        if (
            "batch number" in row_str or "000 1:01" in row_str
        ) and "BATCH_NO" not in header_data:
            val = find_val(row, "000 1:01")
            header_data["BATCH_NO"] = val
        if "tag no" in row_str and "EQUIPMENT_TAG" not in header_data:
            val = find_val(row, "12-17") or find_val(row, "r-17")
    ...
    # 6. Output
    output_file = Path("p24_tables.txt")
    with open(output_file, "w", encoding="utf-8") as out:
        out.write(f"EXTRACTION VERIFICATION FOR P24\n{'=' * 30}\n\n")
    ...
    print(f"P24 verification validated. Results in {output_file.resolve()}")
